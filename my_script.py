...
_text_input_available = False
try:
    from ui.ui_dialog_generic import UiDialogTextInputOkCancel
    from sims4.localization import _create_localized_string
    _text_input_available = True
except:
    pass
...
_pending_story = None
_pending_story_memory_missing = False
_last_inbox_check = 0
_settings = {
    "option_1": False,
    "option_2": False,
...
def _get_settings_path():
    ...
    try:
        ...
        if mods_folder:
            return os.path.join(mods_folder, "AI_Storyteller_Settings.json")
    except:
        pass
    return None
    ...
    try:
        ...
        if "mod_settings" in data:
            for key in _settings:
                if key in data["mod_settings"]:
                    _settings[key] = data["mod_settings"][key]
    except:
        pass
    ...
    try:
        ...
        if os.path.exists(path):
            try:
                ...
                pass
        data["mod_settings"] = dict(_settings)
...
def _get_household_key():
    """用当前可选 Sim 的 id 列表生成唯一 key"""
    try:
        client = services.client_manager().get_first_client()
        if not client:
            return None
        ids = sorted(str(si.sim_id) for si in client.selectable_sims)
        return "_".join(ids) if ids else None
    except:
        return None


def _guess_household_name():
    """取最常见的姓 + '家庭'，找不到则返回 'Household'"""
    try:
        client = services.client_manager().get_first_client()
        if not client:
            return "Household"
        last_names = {}
        for si in client.selectable_sims:
            try:
                ln = si.last_name or ""
                if ln:
                    last_names[ln] = last_names.get(ln, 0) + 1
            except:
                pass
        if last_names:
            best = max(last_names, key=last_names.get)
            return f"{best}家庭"
    except:
        pass
    return "Household"
...
def _load_households():
    """读 JSON → 返回 (active_key, households_dict)"""
    path = _get_settings_path()
    if not path or not os.path.exists(path):
        return (None, {})
    try:
        with open(path, "r", encoding="utf-8") as f:
            data = json.load(f)
        active_key = data.get("active_household", None)
        households = data.get("households", {})
        return (active_key, households)
    except:
        return (None, {})


def _save_households(active_key, households):
    """将 active_household + households 写回 JSON（不覆盖其他字段）"""
    path = _get_settings_path()
    if not path:
        return
    try:
        ...
        if os.path.exists(path):
            try:
                ...
                pass
        data["active_household"] = active_key
...
def _auto_register_household():
    """
    获取当前 household key → 如果不在 JSON 里就新建条目
    同时更新 active_household 字段
    """
    try:
        key = _get_household_key()
        if not key:
            return
        active_key, households = _load_households()
        if key not in households:
            households[key] = {
                "name": _guess_household_name(),
                "profile": "",
                "player_recap": "",
                "ai_recap": "",
            }
        _save_households(key, households)
    except:
        pass
...
# =======================================================
def show_story_notification(text):
    """用通知显示剧情（更宽，内容保留在通知墙）"""
    client = services.client_manager().get_first_client()
    if not client or not client.active_sim:
        return

    notification = UiDialogNotification.TunableFactory().default(
        client.active_sim,
        text=lambda *args: LocalizationHelperTuning.get_raw_text(
            f"<font size='16'>{text}</font>"
        ),
        title=lambda *args: LocalizationHelperTuning.get_raw_text(
            f"AI Storyteller ({MOD_VERSION})"
        ),
        urgency=UiDialogNotification.UiDialogNotificationUrgency.URGENT,
    )
    notification.show_dialog()

def show_story_by_setting(text):
    """根据设置选择弹窗方式"""
    style = _settings.get("popup_style", "notification")

    if style == "dialog":
        show_story_dialog(text)
    elif style == "both":
        show_story_dialog(text)
        show_story_notification(text)
    else:
        # "notification" → 默认
        show_story_notification(text)
...
def show_story_dialog(text):
    """ 弹窗显示剧情（原生宽文字弹窗）"""
    if _text_input_available:
        try:
            _show_native_wide_dialog(f"AI Storyteller ({MOD_VERSION})", text)
            return
        except:
            pass
    # fallback: 原生 OkCancel
    client = services.client_manager().get_first_client()
    if not client: return
    dialog = ui.ui_dialog.UiDialogOkCancel.TunableFactory().default(
        client.active_sim,
        text=lambda *args: LocalizationHelperTuning.get_raw_text(f"<font size='18'>{text}</font>"),
...
def _check_inbox_quick():
    """搭便车在 interaction hook 上的轻量 inbox 检查"""
    global _last_inbox_check, _pending_story

    now = time.time()
    if now - _last_inbox_check < 5:
        return
    _last_inbox_check = now

    try:
        output_dir = get_output_directory()
        signal_path = os.path.join(output_dir, "Story_Ready.signal")
        ...
        log_error(f"Hook inbox check error: {e}", "hook_inbox")
...
def check_inbox_logic(_):
    ...
    try:
        ...
        if content:
            ...
            # 2. 缓存起来（给手动 Show Story 用）
            _pending_story = content

            # 3. 自动弹通知！
            show_story_notification(content)

            # 4. 清空 inbox
            with open(inbox_path, "w", encoding="utf-8") as f:
                f.write("")
        else:
            # inbox 是空的但信号存在，删掉信号避免死循环
            try:
                os.remove(signal_path)
            except:
                pass
    except Exception as e:
        log_error(f"Auto-popup error: {e}", "check_inbox")
...
def do_save_log():
    ...
    try:
        ...
        household_lines = ""
        try:
            hkey = _get_household_key()
            if hkey:
                _, households = _load_households()
                hdata = households.get(hkey, {})
                hname = hdata.get("name", "")
                profile = hdata.get("profile", "")
                recap = hdata.get("player_recap", "")
                if hname:
                    household_lines += f"[Household: {hname}]\n"
                if profile:
                    household_lines += f"[Profile]: {profile}\n"
                if recap:
                    household_lines += f"[My Recap]: {recap}\n"
        except:
            pass
        ...
        if household_lines:
            content_lines.append(household_lines)
        if characters_info:
        ...
        if verify_ok:
            ...
            return (False,
                    f" File created but may be empty!\n\n"
                    f"Path: {path_latest}\n\n"
...
def start_ai_monitor(_connection=None):
    ...
    if _monitor_alarm is not None:
        output(f" AI monitoring is already running! ({MOD_VERSION})")
        return
...
def stop_ai_monitor(_connection=None):
    ...
    if _monitor_alarm is not None:
        ...
        output(" No monitoring is currently running.")
...
def show_path_command(_connection=None):
    ...
    if mods:
        ...
        output(f" Config file: {config}")
        output(f"   Config exists: {os.path.exists(config)}")
    else:
        output(" Could not find Mods folder!")
...
def _log_settings_error(error_msg, context=""):
    """Settings 专用错误日志，写到 settings_error.txt"""
    try:
        error_path = os.path.join(get_output_directory(), "settings_error.txt")
        timestamp = get_log_time()
        with open(error_path, "a", encoding="utf-8") as f:
            f.write(f"{timestamp} [{context}] {error_msg}\n")
    except:
...
# --- Settings 面板 (原生 API，零依赖) ---

def _loc(text):
    """创建 LocalizedString 对象（不是 lambda！）"""
...
def _open_settings_main():
    ...
    try:
        ...
        dialog = UiObjectPicker.TunableFactory().default(
            ...
            title=lambda *args: LocalizationHelperTuning.get_raw_text(
                f"AI Storyteller Settings ({MOD_VERSION})"
            ),
            max_selectable=1,
            min_selectable=1,
        ...
        # 选项 1: Auto
        row1 = ObjectPickerRow(
            option_id=1,
            name=_loc("Auto"),
            row_description=_loc(""),
            is_enable=True,
            tag="auto",
        )
        dialog.add_row(row1)

        # 选项 2: Popup Style（显示当前值）
        current_style = _settings.get("popup_style", "dialog")
        style_display = {"dialog": "Dialog (Classic)", "notification": "Notification (Wide)", "both": "Both"}
        row2 = ObjectPickerRow(
            option_id=2,
            name=_loc(f"Popup Style: {style_display.get(current_style, current_style)}"),
            row_description=_loc("Click to cycle: Dialog > Notification > Both"),
            is_enable=True,
            tag="popup_style",
        )
        dialog.add_row(row2)
        ...
        # 选项 3: Households
        row3 = ObjectPickerRow(
            option_id=3,
            name=_loc("Households"),
            row_description=_loc("View and edit household profiles and recaps"),
            is_enable=True,
            tag="households",
        )
        dialog.add_row(row3)
        ...
        def _on_chosen(d):
            try:
                if not d.accepted:
                    return
                result = d.get_result_tags()
                if not result:
                    return
                chosen = result[0]
                if chosen == "auto":
                    _open_auto_settings()
                elif chosen == "popup_style":
                    # 循环切换
                    cycle = ["dialog", "notification", "both"]
                    current = _settings.get("popup_style", "dialog")
                    idx = cycle.index(current) if current in cycle else 0
                    _settings["popup_style"] = cycle[(idx + 1) % len(cycle)]
                    _save_settings()
                    _open_settings_main()  # 刷新
                elif chosen == "households":
                    _open_households_panel()
            except Exception as e:
                _log_settings_error(f"Main menu error: {e}", "main_on_chosen")

        dialog.add_listener(_on_chosen)
        dialog.show_dialog()

    except Exception as e:
        _log_settings_error(f"Failed to open settings: {e}", "open_settings_main")
        show_story_dialog(f"Settings error: {e}\nCheck settings_error.txt")


def _open_auto_settings():
    """Auto 子面板 — 第二层，3 个开关"""
    try:
        client = services.client_manager().get_first_client()
        if not client or not client.active_sim:
        ...
        dialog = UiObjectPicker.TunableFactory().default(
            client.active_sim,
            text=lambda *args: LocalizationHelperTuning.get_raw_text(
                "Click to toggle ON/OFF. Close to go back."
            ),
            title=lambda *args: LocalizationHelperTuning.get_raw_text(
                "Auto Settings"
            ),
        ...
        for opt_id, key, label in items:
            value = _settings[key]
            status = "ON " if value else "OFF "

            row = ObjectPickerRow(
                option_id=opt_id,
                name=_loc(f"{label}: {status}"),
                row_description=_loc(f"Currently {'enabled' if value else 'disabled'}. Click to toggle."),
                is_enable=True,
                tag=key,
            )
            dialog.add_row(row)
        ...
        def _on_chosen(d):
            try:
                ...
                if result:
                    ...
                    _open_auto_settings()  # 刷新
            except Exception as e:
                _log_settings_error(f"Auto toggle error: {e}", "auto_on_chosen")

        dialog.add_listener(_on_chosen)
        dialog.show_dialog()

    except Exception as e:
        _log_settings_error(f"Failed to open auto settings: {e}", "open_auto_settings")
        show_story_dialog(f" Auto settings error: {e}\nCheck settings_error.txt")


def _open_text_edit(title, current_value, on_save):
    """
    弹出文本输入框，用户编辑后点 OK 触发 on_save(new_text)
    使用原生 _NativeTextInputDialog；不可用时提示去改 JSON
    """
    if not _text_input_available:
        path = _get_settings_path()
        show_story_dialog(
            f"Text input not available.\n"
            f"Please edit directly in:\n{path}"
        )
        return
    try:
        _show_native_wide_dialog(title, "", current_value or "", on_save)
    except Exception as e:
        _log_settings_error(f"Text edit error: {e}", "open_text_edit")
        path = _get_settings_path()
        show_story_dialog(f"Text edit error: {e}\nEdit directly in:\n{path}")


def _open_households_panel():
    """显示所有 households 列表，点击进入详情"""
    try:
        client = services.client_manager().get_first_client()
        if not client or not client.active_sim:
            return
        ...
        if not households:
            show_story_dialog("No households found yet.\nSave a log first to register the current household.")
            return

        dialog = UiObjectPicker.TunableFactory().default(
            client.active_sim,
            text=lambda *args: _loc("Click a household to view/edit details."),
            title=lambda *args: _loc("Households"),
            max_selectable=1,
            min_selectable=1,
            picker_type=UiObjectPicker.UiObjectPickerObjectPickerType.OBJECT,
        )

        for i, (hkey, hdata) in enumerate(households.items()):
            hname = hdata.get("name", "Household")
            label = f"{hname} (Active)" if hkey == active_key else hname
            row = ObjectPickerRow(
                option_id=i + 1,
                name=_loc(label),
                row_description=_loc(hkey),
                is_enable=True,
                tag=hkey,
            )
            dialog.add_row(row)
        ...
        def _on_chosen(d):
            try:
                ...
                result = d.get_result_tags()
                if result:
                    _open_household_detail(result[0])
            except Exception as e:
                _log_settings_error(f"Households panel error: {e}", "households_on_chosen")

        dialog.add_listener(_on_chosen)
        dialog.show_dialog()

    except Exception as e:
        _log_settings_error(f"Failed to open households panel: {e}", "open_households_panel")
        show_story_dialog(f"Households panel error: {e}")


def _open_household_detail(household_key):
    """家庭详情面板：Edit Name / Edit Profile / Edit My Recap / View AI Recap"""
    try:
        client = services.client_manager().get_first_client()
        if not client or not client.active_sim:
            return
        ...
        if household_key not in households:
            show_story_dialog(f"Household not found: {household_key}")
            return

        hdata = households[household_key]
        hname = hdata.get("name", "Household")

        dialog = UiObjectPicker.TunableFactory().default(
            client.active_sim,
            text=lambda *args: _loc(f"Key: {household_key}"),
            title=lambda *args: _loc(hname),
            max_selectable=1,
            min_selectable=1,
            picker_type=UiObjectPicker.UiObjectPickerObjectPickerType.OBJECT,
        )
        ...
        items = [
            (1, "edit_name",    "Edit Name",      hdata.get("name", "")),
            (2, "edit_profile", "Edit Profile",   hdata.get("profile", "")),
            (3, "edit_recap",   "Edit My Recap",  hdata.get("player_recap", "")),
            (4, "view_ai",      "View AI Recap",  hdata.get("ai_recap", "(empty)")),
        ]

        for opt_id, tag, label, value in items:
            preview = (value[:40] + "...") if len(value) > 40 else value
            row = ObjectPickerRow(
                option_id=opt_id,
                name=_loc(label),
                row_description=_loc(preview or "(empty)"),
                is_enable=True,
                tag=tag,
            )
            dialog.add_row(row)
        ...
        def _on_chosen(d):
            try:
                ...
                if chosen == "view_ai":
                    ai_text = hdata.get("ai_recap", "") or "(No AI recap yet)"
                    show_story_dialog(f"AI Recap:\n\n{ai_text}")

                elif chosen == "edit_name":
                    def _save_name(new_text):
                        _, hh = _load_households()
                        if household_key in hh:
                            hh[household_key]["name"] = new_text
                            _save_households(active_key, hh)
                        _open_household_detail(household_key)
                    _open_text_edit("Edit Name", hdata.get("name", ""), _save_name)

                elif chosen == "edit_profile":
                    def _save_profile(new_text):
                        _, hh = _load_households()
                        if household_key in hh:
                            hh[household_key]["profile"] = new_text
                            _save_households(active_key, hh)
                        _open_household_detail(household_key)
                    _open_text_edit("Edit Profile", hdata.get("profile", ""), _save_profile)

                elif chosen == "edit_recap":
                    def _save_recap(new_text):
                        _, hh = _load_households()
                        if household_key in hh:
                            hh[household_key]["player_recap"] = new_text
                            _save_households(active_key, hh)
                        _open_household_detail(household_key)
                    _open_text_edit("Edit My Recap", hdata.get("player_recap", ""), _save_recap)

            except Exception as e:
                _log_settings_error(f"Household detail error: {e}", "household_detail_on_chosen")

        dialog.add_listener(_on_chosen)
        dialog.show_dialog()

    except Exception as e:
        _log_settings_error(f"Failed to open household detail: {e}", "open_household_detail")
        show_story_dialog(f"Household detail error: {e}")


@sims4.commands.Command('ai_settings', command_type=sims4.commands.CommandType.Live)
def open_settings_command(_connection=None):
    """打开设置面板（作弊台备用入口）"""
    output = sims4.commands.CheatOutput(_connection)
    output("Opening AI Storyteller Settings...")
    _open_settings_main()
...
class StartAIInteraction(ImmediateSuperInteraction):
    ...
    def _run_interaction_gen(cls, inst, timeline):
        ...
        if _monitor_alarm is not None:
            show_story_dialog(f" AI monitoring is already running! ({MOD_VERSION})")
            return True
...
class StopAIInteraction(ImmediateSuperInteraction):
    ...
    def _run_interaction_gen(cls, inst, timeline):
        ...
        if _monitor_alarm is not None:
            ...
            show_story_dialog(" No monitoring is currently running.")
...
class ShowStoryInteraction(ImmediateSuperInteraction):
    ...
    @flexmethod
    def _run_interaction_gen(cls, inst, timeline):
        global _pending_story, _pending_story_memory_missing

        if _pending_story:
            story_text = _pending_story
            is_memory_missing = _pending_story_memory_missing
            _pending_story = None
            _pending_story_memory_missing = False

            if is_memory_missing:
                # Memory 缺失 → 显示确认/取消弹窗
                warning_text = (
                    f"<font size='16'> AI did not return a proper memory summary.\n"
                    f"The story was generated but may lack continuity.\n\n"
                    f"──────────────────\n\n"
                    f"{story_text}\n\n"
                    f"──────────────────\n"
                    f"OK = Accept this story\n"
                    f"Cancel = Ask AI to regenerate</font>"
                )
                ...
                def on_retry():
                    ...
                    try:
                        output_dir = get_output_directory()
                        signal_path = os.path.join(output_dir, "Retry_Request.signal")
                        with open(signal_path, "w", encoding="utf-8") as f:
                            f.write("retry")
                        show_story_dialog("<font size='16'> Regeneration requested!\nPlease wait for AI...</font>")
                    except Exception as e:
                        log_error(f"Retry signal error: {e}", "show_story")
                        show_story_dialog(f"<font size='16'> Failed to request retry: {e}</font>")

                show_confirm_dialog(
                    warning_text,
                    f" Story Ready (Memory Incomplete) - {MOD_VERSION}",
                    on_accept,
                    on_retry
                )
            else:
                # 正常显示
                show_story_dialog(f"<font size='16'>{story_text}</font>")
        else:
            ...
            try:
                if os.path.exists(path):
                    with open(path, "r", encoding="utf-8") as f:
                        content = f.read().strip()
                    if content:
                        # 也检查 memory missing 标记
                        if content.startswith("[MEMORY_MISSING]"):
                            content = content.replace("[MEMORY_MISSING]", "", 1).strip()
                        show_story_dialog(f"<font size='16'>{content}</font>")
                        with open(path, "w", encoding="utf-8") as f:
                            f.write("")
                        return True
            except:
                pass
            show_story_dialog("<font size='16'> No new story yet.\nWaiting for AI to write...</font>")
    ...
    def _run_interaction_gen(cls, inst, timeline):
        ...
        def on_cancel():
            try:
                ...
                with open(pending_path, "w", encoding="utf-8") as f:
                    f.write("")
                show_story_dialog(" Events discarded.")
            except:
                pass
...
class SettingsInteraction(ImmediateSuperInteraction):
    """设置面板交互 — 从 Pie Menu 打开"""
    ...
    def _run_interaction_gen(cls, inst, timeline):
        try:
            ...
            _log_settings_error(f"SettingsInteraction error: {e}", "pie_menu")
            show_story_dialog(f" Settings error: {e}")
        return True

class _NativeTextInputDialog(UiDialogTextInputOkCancel):
    """
    原生宽弹窗 — 不需要 S4CL，不需要 XML tuning
    通过继承 UiDialogTextInputOkCancel 并重写 build_msg() 实现
    """
    __qualname__ = '_NativeTextInputDialog'
    ...
    def build_msg(self, additional_tokens=(), icon_override=None, **kwargs):
        msg = super().build_msg(
            additional_tokens=additional_tokens,
            icon_override=icon_override,
            **kwargs
        )
        # 手动注入一个 text_input 字段到 protobuf
        text_input_msg = msg.text_input.add()
        text_input_msg.text_input_name = self.TEXT_INPUT_ID

        if self._initial_value:
            text_input_msg.initial_value = _create_localized_string(
                0, self._initial_value
            )

        # 限制长度（0=不限制）
        text_input_msg.max_length = 0
        text_input_msg.min_length = 0

        return msg


def _show_native_wide_dialog(title, content, initial_input="", on_submit=None):
    """用原生 API 显示宽弹窗"""
    client = services.client_manager().get_first_client()
    if not client or not client.active_sim:
        return
    ...
    dialog = _NativeTextInputDialog.TunableFactory().default(
        sim,
    ...
    if initial_input:
        dialog.set_initial_value(initial_input)

    def _on_response(d):
        if on_submit and d.accepted:
            input_text = ""
            try:
                input_text = d.text_input_responses.get(
                    _NativeTextInputDialog.TEXT_INPUT_ID, ""
                )
            except:
                pass
...
@sims4.commands.Command('test_native_wide', command_type=sims4.commands.CommandType.Live)
def test_native_wide_cmd(_connection=None):
    """测试原生宽弹窗 — 读 inbox 显示"""
    output = sims4.commands.CheatOutput(_connection)
    ...
    if not content:
        content = "Inbox is empty. Put text in Sims4_Inbox.txt first."

    output(f"Showing native wide dialog ({len(content)} chars)...")

    def _on_submit(text):
        output(f"User input: {text[:50]}...")

    try:
        _show_native_wide_dialog(
            f"AI Storyteller ({MOD_VERSION})",
            content,
            "",
            _on_submit
        ...
        output(f"Dialog error: {e}")
        log_error(f"Native wide dialog error: {e}", "test_native_wide")


@sims4.commands.Command('test_native_input', command_type=sims4.commands.CommandType.Live)
def test_native_input_cmd(_connection=None):
    """测试原生输入框 — 带默认文字"""
    output = sims4.commands.CheatOutput(_connection)

    def _on_submit(text):
        output(f"You typed: {text}")

    try:
        _show_native_wide_dialog(
            "Edit Memory",
            "Edit the story memory below:",
            "This is the default text you can edit.",
            _on_submit
        )
    except Exception as e:
        output(f"Dialog error: {e}")